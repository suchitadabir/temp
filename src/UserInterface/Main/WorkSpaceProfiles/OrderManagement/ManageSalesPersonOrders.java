...
public class ManageSalesPersonOrders extends javax.swing.JPanel {
    ...
    public ManageSalesPersonOrders(Business bz, CustomerProfile cp, JPanel jp) {
   ...
   public void populateOrdersTable() {
    ...
    model.setRowCount(0);
    ArrayList<Order> existingOrders = business.getMasterOrderList().getOrders();

    for (Order order : existingOrders) {
        // Check if the order's customer matches the specified customer profile
        if (order.getCustomer().equals(customer)) {
            int orderId = order.getOrderID();
            String status = order.getStatus();
            String customerName = order.getCustomer().getCustomerId(); 
            int salesVolume = order.getOrderTotal();

            Object[] rowData = {orderId, status, salesVolume, customerName};
